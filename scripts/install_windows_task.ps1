...
# next 09:25 ET trigger to local time makes California 06:25 and lets any host
...
$TargetNy = $NowNy.Date.AddHours(9).AddMinutes(25)
...
$Task = New-ScheduledTask `
    ...
    -Settings $Settings `
    -Principal $Principal `
    -Description "ancserAPX pre-trade sync and rebalance at 09:25 America/New_York"
...
Write-Output ("Task '{0}' {1}: next local trigger {2:yyyy-MM-dd HH:mm:ss} ({3}); 09:25 America/New_York." -f `
